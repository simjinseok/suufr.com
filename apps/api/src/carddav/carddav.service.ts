    ...
    if (updates.photoBase64 !== undefined) {
      try {
        ...
        if (student.profileImageUrl) {
          await this.cloudinaryService.deleteByUrl(student.profileImageUrl);
        }

        if (updates.photoBase64) {
          // 새 사진 업로드
          const mediaType = updates.photoMediaType ?? 'image/jpeg';
          const newPhotoUrl = await this.cloudinaryService.uploadPhoto(updates.photoBase64, mediaType);
